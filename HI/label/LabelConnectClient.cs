...
namespace HI.label
{
    internal class LabelConnectClient
    {
        ...
        internal class LabelMessageDecode {
            ...
            private readonly byte[] cache  = new byte[ExpectedDataLength];
        ...
        public void RequestUpdateCount(int index, int number) {
            ...
            byte[] data = [0x08, 0x00, 0x00, 0x00, (byte)index, 0x01, (byte)index, 0x04, 0x00, high, low, 0x00, (byte)count];
             this.SendMessage(data);
        }
        public void RequestOpenLight(int index, int number) {
            int count = 1 + index + 1 + number;
            byte[] data = [0x08, 0x00, 0x00, 0x00, (byte)index, 0x01, (byte)index, 0x01, 0x00, 0x00, (byte)number, 0x00, (byte)count];
            this.SendMessage(data);
        }
        public void RequestCloseLight(int index) {
            int count = 1 + index + 5;
            byte[] data = [0x08, 0x00, 0x00, 0x00, (byte)index, 0x01, (byte)index, 0x05, 0x00, 0x00, 0x00, 0x00, (byte)count];
            this.SendMessage(data);
        }
        public void RequestStatus(int index) {
            int count = 1 + index + 6;
            byte[] data = [0x08, 0x00, 0x00, 0x00, (byte)index, 0x01, (byte)index, 0x06, 0x00, 0x00, 0x00, 0x00, (byte)count];
            this.SendMessage(data);
        }
